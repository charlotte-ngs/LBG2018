...
## Accurracy {#accurracy}
The accuracy for a BLUP-based animal model is no longer as easily derived as with the selection index. Our animal model is a linear mixed effects model containing fixed and random effects. Due to the properties of BLUP-based methods, the estimates of the fixed effects and the prediction of the random effects have minimum error variance. For the fixed effects, this error variance can be computed as

$$var(\beta - \hat{\beta}) = var(\hat{\beta})$$

because fixed effects $\beta$ do not have any variance. For the random effects $a$ the prediction error variance (PEV) does not simplify to the variance of the predicted effects $\hat{a}$. Random effects by their nature do have a certain variance which is part of the model specification. For a BLUP animal model the variance of the random effects $a$ correspond to $var(a) = A * \sigma_a^2$. Any meaningful prediction $\hat{a}$ of a random effect $a$ should also satisfy that the variance $var(\hat{a})$ predicts $var(a)$ as closely as possible. Following this argument $var(\hat{a})$ cannot correspond to the prediction error variance. The prediction error variance $PEV(\hat{u})$ is computed as 
...
Because the vector $\hat{\beta}$ has length $p$, the first $p$ components in $s$ correspond to estimates of fixed effects. The remaining $q$ components of $s$ correspond to the $q$ predicted breeding values of vector $\hat{a}$. Let us assume that we want to have a closer look at how the predicted breeding value $\hat{a}_i$ of the animal at position $i$ in the vector $\hat{a}$. The component $\hat{a}_i$ can be found on position $p+i$ in the vector $s$. As a consequence of that the $(p+i)$-th line in $M$ contains the coefficients that are relevant for the computation of the predicted breeding value $\hat{a}_i$. These coefficients determine what type of information is used to compute $\hat{a}_i$. In what follows, we describe how these coefficients are determined.
...
\begin{tabular}{lll}
where    &  $\alpha$  &  ration between variance components $\sigma_e^2 / \sigma_a^2$  \\
         &  $\delta^(j)$  &  contribution for animal $j$ to $A^{-1}$
\end{tabular}


\vspace{2ex}
Solving \@ref(eq:YiDecompEq) for $\hat{a}_i$ leads to
...
From the decomposition in \@ref(eq:AhatDecompEq), we can see that the predicted breeding value $\hat{a}_i$ consists of the following components

* Predicted breeding values $\hat{a}_s$ and $\hat{a}_d$ of parents $s$ and $d$ of $i$
* Own performance $y_i$ of $i$
* Predicted breeding values $\hat{a}_{k_j}$ and $\hat{a}_{l_j}$ of progeny $k_j$ and mates $l_j$
