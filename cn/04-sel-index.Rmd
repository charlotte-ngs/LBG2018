...
The objective of the index construction is to maximize the correlation $r_{HI}$ between the index $I$ and the aggregate genotype $H$. Because the index $I$ corresponds to a multiple linear regression, the mean squarred error between aggregate genotype and index is to be minimized. From this it follows that 
...
For our case with the progeny records, $t$t takes the value of ${1\over 4}h^2$. For more details on how to compute $\sigma_{\bar{y}}^2$, see section \@ref(app-deriv-variance-mean-progeny-performance).



<!-- Start of Appendix here -->
## Appendix: Derivation of Index Normal Equations {#derivation-index-normal-equation}
In this section we want to show how to derive the index normal equations from the objective criterion in the index construction procedure. The objective criterion was formualted in equation \@ref(eq:indexconstructionobjective) as 
...
## Appendix: Derivation of Variance of Mean Progeny Performance {#app-deriv-variance-mean-progeny-performance}
