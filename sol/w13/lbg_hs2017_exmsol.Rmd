...
The following elements contain $F_5$
$$(2,4), (4,2), (5,5)$$
...
Because only $1$ and $2$ are not related, these would be the only parents with which $F_5 = 0$ would be true.
...
The estimated residual variance corresponds to the squared residuals divided by the degrees of freedom $df_e = n - p$ where $n$ corresponds to the number of observations and $p$ corresponds to the number of herd effects. Therefore $df_e = `r n_nr_obs` - `r length(coefficients(lm_gewicht))` = `r n_nr_obs - length(coefficients(lm_gewicht))`$. 
...
- With the Maximum-Likelihood estimate the degrees of freedom $p$ for the estimation of the fixed effects are not considered
- The value of the estimated residual variance is the same as in a), but the factor with which the sums of squares are multiplied is $1/n$ and not $1/(n-p)$. Hence, we get 
$$\widehat{\sigma^2_{ML}} = `r crossprod(vec_residuals)/n_nr_obs`$$ 
- The estimate computed in a) based on the residuals is unbiased and is considered better.
...
\vspace{3ex}
```{r, echo=FALSE}
dfPopSol <- data.frame(Population = c("Wildlife", "Lifestock"),
                    Selektion  = c("Natural selection based on environment", "artificial and directed selection"),
                    Anpaarung  = c("random mating", "planned mating"))
...
where
```{r, echo=FALSE}
dfSol <- data.frame(Abbreviation = c("ED", "SG", "RD","sEltern", "sNachkommen", "NKD"),
                    Meaning = c("parent average",
                                  "selection threshold",
                                  "average of selected parents",
                                  "standard deviation of parents",
                                  "standard deviation of progeny",
                                  "average of progeny"))
knitr::kable(dfSol)
```
...
The inbreeding coefficients are computed based on the sum of the squared values of each row of matrix $R$
...
The predicted breeding values based on the own performance can be computed as
$$\hat{a}_i = h^2(y_i - \mu)$$

Applying this to the data from the above table, this means

```{r, echo=FALSE, results='asis'}
dfZwsEl <- data.frame(Animal = dfMlrData$Animal,
                      PBV = h2 * (dfMlrData$Observation - mu))
knitr::kable(dfZwsEl)
```
...
To set up the coefficient matrix, we need to know the incidence matrices $X$ and $Z$.
...
```{r}
(y <- dfMlrData$Observation[!is.na(dfMlrData$Observation)])
```
...
* In a) only animals with an own performance get a predicted breeding value. In b) all animals get a predicted breeding value.
* In a) the population mean is used to account for environmental effects, in b) also herd effects are accounted for.
