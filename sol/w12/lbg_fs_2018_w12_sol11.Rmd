...
vec_a <- rmddochelper::vecGetVecElem(psBaseElement = "a", pnVecLen = nNrObsSmd, psResult = "latex")
vec_hat_a <- rmddochelper::vecGetVecElem(psBaseElement = "\\hat{a}", pnVecLen = nNrObsSmd, psResult = "latex")
...
The matrix $G^{-1}$ is computed as $G^{-1} = \lambda * A^{-1}$ where $\lambda = \sigma_e^2 / \sigma_a^2 = `r n_sigmae2`/`r n_sigmaa2` = `r lambda`$ and $A$ corresponds to the numerator relationship matrix. Its inverse $A^{-1}$ is 
...
$$\hat{\mu} - 3 \hat{a}_1 - 3 \hat{a}_2 + 1.5 \hat{a}_3 + 8.5 \hat{a}_4 - 3 \hat{a}_5 = y_4$$

Solving for $\hat{a}_4$ gives

$$\hat{a}_4 = {1\over 8.5} \left[y_4 - \hat{\mu} + 3 \hat{a}_1 + 3 \hat{a}_2 - 1.5 \hat{a}_3 + 3 \hat{a}_5 \right]$$
...
For the animal model, this shows that with the animal model all available information is used to predict the breeding value $\hat{a}_4$. The following information is contained in the above equation

* own performance record $y_4$
* the estimate $\hat{\mu}$
* the predicted breeding values $\hat{a}_1$ and $\hat{a}_2$ of the parents
* the predicted breeding value $\hat{a}_3$ of the mate and 
* the predicted breeding value $\hat{a}_5$.
